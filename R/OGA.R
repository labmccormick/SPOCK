...
create.plots<-function(locationofRAW=getwd(),measureInterval=15)
{
  x<-y<-ypre<-yfiltered<-ycorrected<-NULL
...
#' @param method a character string specifying the filtering method applied to RAW growth curves, must be either "BUTTERWORTH" (default) or "LOESS"
...
################################# Handling of filtering method
#allow for LOESS filtering
      ...
      # Manipulate data one sample column at a time
        ...
        for (ii in 2:length(truewells))
            # for ii in 2:the number of columns containing RAW SAMPLE DATA
          {
            ...
            #User selects filtering method from BUTTERWORTH or LOESS
            ...
            {
            yfilterd<- lowess(c(1:length(y)),y)$y
            } else if (method!="BUTTERWORTH" && method !="LOESS")
            {

                print(paste0("Filtering method not defined, or invalid. method= (default butterworth) "))
                return(-1)
            }
           ...
           # Housekeeping
            ...
            if(method=="BUTTERWORTH")
            {rm(mirroredy, filteredmirroedy)}
            rm(ycorrected, yfilterd, ul, ymblk, y)
          }
          close(pb)
          setwd(homedir)
          print("Complete")
...
# write out all values to DATASET.csv results folder including DT data
    ...
    if(create.plot)
    {
      create.plots(locationofRAW=RAW, measureInterval=measureInterval)
    }
    # End
  }
